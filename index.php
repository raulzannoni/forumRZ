<?php
    ...
    if(isset($_GET['ctrl'])) {$ctrlname = $_GET['ctrl'];}
    ...
    if(!class_exists($ctrlNS)) {$ctrlNS = "controller\\".DEFAULT_CTRL."Controller";}
    ...
    if(isset($_GET['action']) && method_exists($ctrl, $_GET['action'])) {$action = $_GET['action'];}
    ...
    if($action == "ajax") {echo $result;}
    else
        {
            //démarre un buffer (tampon de sortie)
            ob_start();
